...
class LinearSpline(torch.nn.Module):
    ...
    def integrate(x_knots: torch.Tensor, y_knots: torch.Tensor, t: torch.Tensor):
        ...
        # Integral of previous segments + rectangular area + triangular area
        # delta = t - x_knots[k] is the offset within the current segment
        integral = C_k + h_k * d_t.unsqueeze(0) + s_k / 2 * d_t.unsqueeze(0) ** 2  # (D,B)
