...
true_params = Params(
    ...
    alpha=torch.tensor(
        [
            [0.5, 0.2],  # Impact from type j to type i
            [0.1, 0.4],
        ]
    ),
    beta=torch.tensor([[0.5, 2.0], [0.2, 0.8]]),
)
fake_params = Params(
    mu=torch.tensor([0.5, 0.1]),  # Baseline for each type
    alpha=torch.tensor(
        [
            [0.2, 0.5],  # Impact from type j to type i
            [0.4, 0.1],
        ]
    ),
    beta=torch.tensor([[0.2, 0.5], [0.8, 0.2]]),
)
...
with torch.no_grad():
    ...
    for d in range(D):
        ...
        axes[0].scatter(
            events.time_points[mask],
            np.ones_like(events.time_points[mask]) * (d + 0.1),
            c="red",
            label=f"True Events type {d}",
            marker="o" if d == 0 else "x",
            alpha=0.6,
        )
        axes[0].scatter(
            fake_events.time_points[mask_fake],
            np.ones_like(fake_events.time_points[mask_fake]) * (d - 0.1),
            c="blue",
            marker="o" if d == 0 else "x",
            label=f"Fake Events type {d}",
            alpha=0.6,
        )
        # Intensities
    ...
    timeline_ll_true = [
        ...
        / t
        for t in timeline_evalp
    ]
    timeline_ll_fake_data = [
        true_model.likelihood(
            ts=MVEventData(
...
for i, events in enumerate(all_events[:subset_datashowcase]):
    for d in range(D):
        mask = events.event_types == d
        ax.plot(events.time_points[mask], np.ones_like(events.time_points[mask]) * i, "o", color=colors[d], alpha=0.4)
...
def fit_model_basic_mv(
    model: MultiVariateHawkesProcess,
    events_batch: list[MVEventData],
    T: float,
    test_events: Optional[list[MVEventData]] = None,
    eval_freq: float = 10,
    num_steps=100,
    step_size=0.01,
...
test_params = Params(
    ...
    alpha=torch.tensor([[0.2, 0.1], [0.1, 0.2]]),
    beta=torch.tensor([[0.3, 0.2], [0.2, 0.3]]),
)
...
true_ll_train = torch.mean(
    torch.stack([true_model.likelihood(ts=ev, T=T, num_integration_points=0) for ev in all_events])
)
init_ll_train = torch.mean(
    torch.stack([init_model.likelihood(ts=ev, T=T, num_integration_points=0) for ev in all_events])
)
...
true_ll_test = torch.mean(
    torch.stack([true_model.likelihood(ts=ev, T=T, num_integration_points=0) for ev in all_events_test])
)
init_ll_test = torch.mean(
    torch.stack([init_model.likelihood(ts=ev, T=T, num_integration_points=0) for ev in all_events_test])
)
...
step_size = 1e-3
batch_size = 100
fit_model, likelihoods, test_likelihoods = fit_model_basic_mv(
    model=fit_model.cuda(),
    events_batch=[e.cuda() for e in all_events],
    T=torch.tensor(T).cuda(),
    test_events=[e.cuda() for e in all_events_test],
    num_steps=num_steps,
    step_size=step_size,
...
model_ll_train = torch.mean(
    torch.stack([fit_model.likelihood(ts=ev.cuda(), T=T, num_integration_points=0) for ev in all_events])
).cpu()
model_ll_test = torch.mean(
    torch.stack([fit_model.likelihood(ts=ev.cuda(), T=T, num_integration_points=0) for ev in all_events_test])
).cpu()
...
axes[0].hlines(
    model_ll_train.item(), xmin=0, xmax=num_steps, colors="green", linestyles="dashed", label="Final Model LL"
)
...
axes[0].legend()
axes[1].plot(
    np.linspace(0, num_steps, len(test_likelihoods)),
    [l / len(all_events_test) for l in test_likelihoods],
    marker="x",
    markersize=2,
)
axes[1].hlines(
    true_ll_test.item(), xmin=0, xmax=num_steps, colors="red", linestyles="dashed", label="True Model LL Test"
)
axes[1].hlines(
    init_ll_test.item(), xmin=0, xmax=num_steps, colors="blue", linestyles="dashed", label="Start Model LL Test"
)
axes[1].hlines(
    model_ll_test.item(), xmin=0, xmax=num_steps, colors="green", linestyles="dashed", label="Final Model LL Test"
)
...
with torch.no_grad():
    eval_points = torch.linspace(0, stop, num_eval)
    intensity_values_true = torch.stack([true_model.intensity(t, events) for t in eval_points])  # Shape (num_eval, D)
    intensity_values_model = torch.stack(
        [fit_model.cpu().intensity(t, events) for t in eval_points]
    )  # Shape (num_eval, D)
    intensity_values_init = torch.stack(
        [init_model.cpu().intensity(t, events) for t in eval_points]
    )  # Shape (num_eval, D)
    ...
    for d in range(D):
        ...
        axes[0].scatter(
            events.time_points[mask],
            np.zeros_like(events.time_points[mask]) + d * 0.1,
            c=colors[d],
            label=f"True Events type {d}",
            alpha=0.6,
        )
        ...
        axes[d + 1].plot(
            eval_points,
            intensity_values_model[:, d],
            c=colors[d],
            label=f"Intensity type {d} (Model)",
            linestyle="dashed",
        )
        axes[d + 1].plot(
            eval_points,
            intensity_values_init[:, d],
            c=colors[d],
            label=f"Intensity type {d} (Init values)",
            linestyle="dotted",
        )
...
# %%
aa = torch.mean(
    torch.stack([fit_model.cuda().likelihood(ts=ev.cuda(), T=100, num_integration_points=0) for ev in add_test_dataset])
).cpu()
bb = torch.mean(
    torch.stack(
        [true_model.cuda().likelihood(ts=ev.cuda(), T=100, num_integration_points=0) for ev in add_test_dataset]
    )
).cpu()
cc = torch.mean(
    torch.stack(
        [init_model.cuda().likelihood(ts=ev.cuda(), T=100, num_integration_points=0) for ev in add_test_dataset]
    )
).cpu()
print(aa, bb, cc)
# %%
