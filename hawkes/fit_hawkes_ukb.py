...
def basic_train_loop(model:MultiVariateHawkesProcess, events_batch:DataLoader, T:float, test_events:Optional[DataLoader]=None, num_steps=100, step_size=0.01, eval_freq:float=10, save_file_name:Optional[str]=None):
            ...
            if test_events is not None and (step % eval_freq == 0 or step == num_steps-1):
                with torch.no_grad():
                    ...
                    for test_batch in test_events:
                        ...
                        batch_ll = batch_ll_mv(model, test_batch, T)
                        test_lls.append(batch_ll)
...
train_batches = [all_events_real[i] for i in train_indices]
validation_batches = [all_events_real[i] for i in validation_indices]
test_batches = [all_events_real[i] for i in test_indices]
#%%
...
dataset_val = ListDataset(validation_batches)
dataset_test = ListDataset(test_batches)

dataloader_train = DataLoader(dataset=dataset_train, batch_size=BATCH_SIZE, shuffle=True, collate_fn=lambda x: x)
dataloader_val = DataLoader(dataset=dataset_val, batch_size=BATCH_SIZE, shuffle=False, collate_fn=lambda x: x)
dataloader_test = DataLoader(dataset=dataset_test, batch_size=BATCH_SIZE, shuffle=False, collate_fn=lambda x: x)
...
if load_path is not None and load_path.exists():
    ...
    loaded_model = MultiVariateHawkesProcess(None, D)
    loaded_model.load_state_dict(torch.load(str(load_path)))
    real_MVHP = loaded_model
else:
    real_MVHP = MultiVariateHawkesProcess(None, D, seed=43)

cuda_real_MVHP = real_MVHP.cuda()
#%%

init_ll_train = torch.mean(torch.stack([real_MVHP.likelihood(ts=ev, T=T, num_integration_points=0) for ev in train_batches]))
init_ll_val = torch.mean(torch.stack([real_MVHP.likelihood(ts=ev, T=T, num_integration_points=0) for ev in validation_batches]))
init_ll_test = torch.mean(torch.stack([real_MVHP.likelihood(ts=ev, T=T, num_integration_points=0) for ev in test_batches]))
...
# %%
with torch.no_grad():
    ll = torch.sum(torch.stack([cuda_real_MVHP.likelihood(all_events_real[i].cuda(), T=torch.tensor(data=T).cuda(), num_integration_points=0) for i in range(1000)]))
...
fit_model, train_lls, val_lls = basic_train_loop(cuda_real_MVHP, dataloader_train,torch.tensor(T).cuda(),  dataloader_val, num_steps=num_steps, step_size=step_size, save_file_name="models/latest.pth", )
#%%
