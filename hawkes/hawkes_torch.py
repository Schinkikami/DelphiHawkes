...
class MultiVariateHawkesProcess(torch.nn.Module):
    ...
    def __init__(self, params:Optional[Params], D:Optional[int]=None, seed:Optional[int]=42, reg_lambda:Optional[float]=None, integration_mode:Literal["trapezoidal", "monte_carlo", "mc_trapezoidal"]="trapezoidal"):
    ...
    def _positive_likelihood_vectorized(
        ...
        interaction_terms = torch.tril(interaction_terms, diagonal=-1)  # Zero out diagonal and above, to ensure causality.
    ...
    def _integral_numerical(self, T:float, ts:MVEventData, num_integration_points:int=1000):
        ...
        if self.INTEGRATION_MODE == "trapezoidal":
            ...
            if not hasattr(self, '_mc_warning_issued'):
                print("Warning: Monte Carlo integration mode is not recommended due to high variance and unoptimized implementation.")
