...
def load_model(model_path: Path, model_type: str, D: int = 1268, num_knots: int = 15, delta_t: float = 0.1):
    ...
    if model_type == "poisson":
        model = PoissonProcess(D=D, seed=42)
    elif model_type == "inhomogeneous":
        model = ConditionalInhomogeniousPoissonProcess(D=D, seed=42)
    elif model_type == "spline":
        model = SplinePoissonProcess(D=D, num_knots=num_knots, delta_t=delta_t, seed=42)
    else:
...
def plot_intensity_curves(model, time_range, ts: BatchedMVEventData):
    intensities = []

    xs = torch.linspace(time_range[0], time_range[1], 200)

    with torch.no_grad():
        for t in xs:
            intensity = model.intensity(t.unsqueeze(0), ts)  # Shape: (1, D)
            intensities.append(intensity)
    intensity_tensor = torch.cat(intensities, dim=0)  # Shape: (T, D)

    for d in range(intensity_tensor.shape[1]):
        plt.plot(xs.numpy(), intensity_tensor[:, d].numpy(), label=f"Dimension {d}")

    # plt.plot(xs.numpy(), intensity_tensor.sum(dim=1).numpy())
    plt.xlabel("Time")
    plt.ylabel("Marginal Intensity")
    plt.title("Intensity Curve")
    plt.show()


def plot_cumulative_intensity_curves(model, time_range, ts: BatchedMVEventData):
    intensities = []
    ...
    with torch.no_grad():
        for t in xs:
            intensity = model.cumulative_intensity(t.unsqueeze(0), ts)  # Shape: (1, D)
            intensities.append(intensity)
    intensity_tensor = torch.cat(intensities, dim=0)  # Shape: (T, D)
